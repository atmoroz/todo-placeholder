	...
	return(
		<form onSubmit={submitForm} className='postForm'>
			<label htmlFor="titleInput" className='postLabel'>
				Title:
				<input name='titleInput' type="text" value={titlePost} onChange={changePost} className="postInput"/>
			</label>
			<label htmlFor="bodyInput" className='postLabel'>
				Body:
				<textarea name='bodyInput' type="text" value={bodyPost} onChange={changePost} className='postTextarea' />
			</label>
			<button className="postButton" onClick={submitForm}>Change</button>
		</form>
	)
}
